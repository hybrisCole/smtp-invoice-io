...
pub async fn process(
    invoice_list: &Vec<Vec<u8>>,
) -> Result<(Vec<FacturaElectronica>, Vec<MensajeHacienda>)> {
    ...
    for invoice in invoice_list {
        ...
        let parsed_invoice = parser::parse(&xml_invoice).expect("<root>No XML</root>");
        let document = parsed_invoice.as_document();
        let factory = Factory::new();
        let xpath_invoice = factory
            .build(ELECTRONIC_INVOICE)
            .expect("Could not compile XPath");
        let xpath_invoice = xpath_invoice.expect("No XPath was compiled");
        let context = Context::new();
        let value_xpath_invoice = xpath_invoice
            .evaluate(&context, document.root())
            .expect("XPath evaluation failed");
        if (&value_xpath_invoice.string() != "") {
            let factura: FacturaElectronica =
                from_str(&xml_invoice).expect("Not a FacturaElectronica XML");
            vec_factura_electronica.push(factura);
        } else {
            let xpath_electronic_message = factory
                .build(ELECTRONICE_MESSAGE)
                .expect("Could not compile XPath");
            let xpath_electronic_message = xpath_electronic_message.expect("No XPath was compiled");
            let context = Context::new();
            let value_xpath_electronic_message = xpath_electronic_message
                .evaluate(&context, document.root())
                .expect("XPath evaluation failed");
            if (&value_xpath_electronic_message.string() != "") {
                let mensaje_hacienda: MensajeHacienda = from_str(&xml_invoice).unwrap();
